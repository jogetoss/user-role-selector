...
<div id="userRoleSelections" style="display: none;">
    <#list options as option>
        <#assign switchUrl = "/jw/web/userview/${appId!}/${userviewId!}/${option.id!}">
        <#if menuId?? && (menuId?trim?length > 0)>
            <#assign safeMenuId = menuId?trim?replace("^/+", "", "r")>
            <#assign switchUrl = switchUrl + "/" + safeMenuId>
        </#if>

        <li>
            <a class="switchRole"
               title="Switch to ${option.label!?html}"
               href="#"
               onClick="window.location='${switchUrl}';return false;">
